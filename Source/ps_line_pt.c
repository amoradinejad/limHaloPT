 ...
 * -# PS_line()               computes tree-level line power spectrum in real-space
...
double PS_line(struct Cosmology *Cx, double k, double z, size_t line_id)
{
      ...
      double f = pow(Tave_line,2.)*pow(b1z,2.)*Pk_dlnPk(Cx, k, z, LPOWER);
...
double PS_line_RSD(struct Cosmology *Cx, struct Cosmology *Cx_ref, double k, double mu, double z, size_t line_id)
{
      ...
      double growth_rate = growth_f(Cx, z);
      ...
      f = AP_fac * sup_fac * a0_P * pow(Tave_line, 2.)*pow(b1z, 2.) * Pk_dlnPk(Cx, k, z, LPOWER);
